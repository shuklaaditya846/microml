...
#define ACCEL_THRESHOLD 13 //Adjust this to change when the detection should trigger
...
void recordIMU() {
  ...
  for (int i = 0; i < NUM_SAMPLES; i++) {
    ...
    gx = constrain(a.acceleration.x - baseline[0], -TRUNCATE, TRUNCATE);
    gy = constrain(a.acceleration.y - baseline[1], -TRUNCATE, TRUNCATE);
    gz = constrain(a.acceleration.z - baseline[2], -TRUNCATE, TRUNCATE);
    ax = constrain(g.acceleration.x - baseline[0], -TRUNCATE, TRUNCATE);
    ay = constrain(g.acceleration.y - baseline[1], -TRUNCATE, TRUNCATE);
    az = constrain(g.acceleration.z - baseline[2], -TRUNCATE, TRUNCATE);
...
void classify() {
  ...
  else if (gesture == "right_swipe")
  {
    relayStatus_2 = !relayStatus_2;
    myData.relay2 = relayStatus_2;
  }
  else if (gesture == "SlamGesture") //slam gesture not trained
...
//  //ALTERNATE METHOD : setupt motion detection
...
void loop() {
  ...
  ax = constrain(g.gyro.x - baseline[0], -TRUNCATE, TRUNCATE);
  ay = constrain(g.gyro.y - baseline[1], -TRUNCATE, TRUNCATE);
  az = constrain(g.gyro.z - baseline[2], -TRUNCATE, TRUNCATE);
